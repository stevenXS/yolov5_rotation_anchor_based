...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=True, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rank=-1, workers=8, image_weights=False, quad=False, prefix='', cache_name=None): # add cache_name
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',cache_name=None): # add cache_name
        self.img_size = img_size
        ...
        cache_path = (p if p.is_file() else Path(self.label_files[0]).parent).with_name(cache_name).with_suffix('.cache')# add,修改适配不同yaml文件的数据集cache文件
