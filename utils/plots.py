...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path('runs/visualization')):
    """
    ...
    # if 'C3TR' or 'Detect' in module_type :
    ...
    if 'C3TR' or 'SubPix' in module_type:
        batch, channels, height, width = x.shape  # batch, channels, height, width
        ...
        if height > 1 and width > 1:
            ...
            # save image
            make_grid(tensor=x*255, nrow=32, padding=2,normalize=False)
            save_image(x[0].detach().cpu().unsqueeze(1), save_dir/f, nrow=32, normalize=False)
            # cv2.imwrite(f'img_{si}.jpg', 255 * xi[0].cpu().numpy().transpose((1, 2, 0))[:, :, ::-1])  # save
            # cv2.imwrite(f'{f}.jpg', 255 * x[0].cpu().numpy().transpose((1, 2, 0))[:, :, ::-1])  # save
            print(f'Saving {save_dir / f}... ({n}/{channels})')
