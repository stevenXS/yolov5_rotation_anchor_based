...
def dota2LongSideFormat(imgpath, txtpath, dstpath, extractclassname):
    ...
    for fullname in filelist:  # fullname='/.../P000?.txt'
        ...
        if num_gt == 0:
            # TODO:不删除图片和标签，2021年11月24日20:15:11
            # os.remove(os.path.join(dstpath, name + '.txt'))  #
            # os.remove(img_fullname)
            # os.remove(fullname)
            # print('%s 图片对应的txt不存在有效目标,已删除对应图片与txt' % img_fullname)
            print('%s 图片对应的txt不存在有效目标' % img_fullname)
    print('已完成文件夹内DOTA数据形式到长边表示法的转换')

# TODO add， 2021年11月24日20:11:10
def dota2LongSideFormatNonNormalization(imgpath, txtpath, dstpath, extractclassname):
    """
    trans dota farmat to longside format
    :param imgpath: the path of images
    :param txtpath: the path of txt in dota format
...
if __name__ == '__main__':
    ## an example
    # TODO 长边标注格式,归一化版本，用于训练
    # dota2LongSideFormat('./DOTA_demo/images', # 源数据图片路径
    #                     './DOTA_demo/labelTxt', # 源数据标签路径
    #                     './DOTA_demo/yolo_labels', # 保存结果后的路径
    #                     util.classnames_v1_5) # 类别名

    # TODO 长边标注格式，非归一化版本，用于cocoapi评估
    dota2LongSideFormatNonNormalization('/home/fofo/A/xsq/datasets/dota-v15/non-normal-dota/val_dota/images',  # 源数据图片路径
                        '/home/fofo/A/xsq/datasets/dota-v15/non-normal-dota/val_dota/labelTxt',  # 源数据标签路径
                        '/home/fofo/A/xsq/datasets/dota-v15/non-normal-dota/val_dota/labels',  # 保存结果后的路径
                        util.classnames_v1_5)  # 类别名
